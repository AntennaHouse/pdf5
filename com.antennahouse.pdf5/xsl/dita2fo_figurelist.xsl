...
>
        ...
        <fo:block>
            ...
            <fo:block>
                ...
                <fo:marker marker-class-name="{$cTitleBody}">
                    <fo:inline><xsl:copy-of select="$cFigureListTitle"/></fo:inline>
                </fo:marker>
    ...
    <xsl:template match="*[contains(@class,' map/topicref ')][@href]" mode="MAKE_FIGURE_LIST">
        ...
        <xsl:for-each select="$topicContent/descendant::*[contains(@class, ' topic/fig ')][child::*[contains(@class, ' topic/title ')]]">
            ...
            <xsl:variable name="figId" select="if (@id) then string(ahf:getIdAtts($fig,$topicRef,true())) else ahf:generateIdByTrCount($fig,$topicRefCount)" as="xs:string"/>
    ...
    <!-- 
     ...
     note:		Changed to use fo:list-block. 2011-09-09 t.makita
     -->
    <xsl:template name="makeFigListLine">
        ...
        <fo:list-block>
            <xsl:copy-of select="ahf:getAttributeSet('atsFigListBlock')"/>
            <fo:list-item>
