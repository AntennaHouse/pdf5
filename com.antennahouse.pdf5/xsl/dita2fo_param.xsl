...
<xsl:stylesheet version="2.0" xmlns:fo="http://www.w3.org/1999/XSL/Format"
    ...
    <xsl:variable name="pAssumeSortasPinyin" select="boolean($PRM_ASSUME_SORTAS_PINYIN=$cYes)"
        as="xs:boolean"/>
    ...
    <xsl:variable name="pIncludeFrontmatterToToc"
        select="boolean($PRM_INCLUDE_FRONTMATTER_TO_TOC=$cYes)" as="xs:boolean"/>
    ...
    <xsl:variable name="pAddNumberingTitlePrefix"
        select="boolean($PRM_ADD_NUMBERING_TITLE_PREFIX=$cYes)" as="xs:boolean"/>

    <!-- Add part/chapter to title
     -->
    <xsl:param name="PRM_ADD_PART_TO_TITLE" select="$cYes"/>
    <xsl:variable name="pAddPartToTitle"
        select="boolean($PRM_ADD_PART_TO_TITLE=$cYes) and $pAddNumberingTitlePrefix" as="xs:boolean"/>

    <!-- Add thmbnail index
      -->
    <xsl:param name="PRM_ADD_THUMBNAIL_INDEX" select="$cYes"/>
    <xsl:variable name="pAddThumbnailIndex" select="boolean($PRM_ADD_THUMBNAIL_INDEX=$cYes)"
        as="xs:boolean"/>
    ...
    <xsl:variable name="pOutputDraftComment" select="boolean($PRM_OUTPUT_DRAFT_COMMENT=$cYes)"
        as="xs:boolean"/>

    <!-- Output required-cleanup -->
    <xsl:param name="PRM_OUTPUT_REQUIRED_CLEANUP" select="$cNo"/>
    <xsl:variable name="pOutputRequiredCleanup" select="boolean($PRM_OUTPUT_REQUIRED_CLEANUP=$cYes)"
        as="xs:boolean"/>
    ...
    <xsl:variable name="pSortGlossEntry" select="boolean($PRM_SORT_GLOSSENTRY=$cYes)"
        as="xs:boolean"/>
    ...
    <xsl:variable name="pDisplayFnAtEndOfTopic"
        select="boolean($PRM_DISPLAY_FN_AT_END_OF_TOPIC=$cYes)" as="xs:boolean"/>
    ...
    <xsl:variable name="pOutputTableTitleAfter"
        select="boolean($PRM_OUTPUT_TABLE_TITLE_AFTER=$cYes)" as="xs:boolean"/>

    <!-- Output plug-in start message.
         2013-09-30 t.makita
      -->
    <xsl:param name="PRM_OUTPUT_START_MESSAGE" select="$cYes"/>
    <xsl:variable name="pOutputStartMessage" select="boolean($PRM_OUTPUT_START_MESSAGE=$cYes)"
        as="xs:boolean"/>
